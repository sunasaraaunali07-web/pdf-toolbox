 ...
 * Compress PDF page
 *
 * UI: Drop zone → file list → "Compress All" → per-file results with download.
 * No user-facing compression mode or quality selection — the service pipeline
 * handles strategy selection automatically.
 */

import { compressBatch } from '../services/pdfCompress';
import {
  validatePdfFile,
...
<div class="tool-page">
  <div class="container">
    <header class="tool-page-header">
      ...
      <p class="tool-page-desc">Reduce PDF file size while keeping text selectable. Process up to ${MAX_COMPRESS_FILES} files at once. All processing happens locally in your browser.</p>
    </header>
    ...
    try {
      ...
      resultList.innerHTML = results.map(r => {
        if (r.result) {
          ...
          const { data, originalSize, compressedSize } = r.result;
          ...
          const saved = compressedSize < originalSize;
          return /* html */ `
            <div class="result-item">
              <div class="result-item-body">
                <div class="result-item-name">${filename}</div>
                <div class="result-item-meta">${formatBytes(originalSize)} → ${formatBytes(compressedSize)}</div>
                <div class="result-item-meta ${saved ? 'saved' : ''}">${ratio}${r.result.technique && saved ? ` · ${r.result.technique}` : ''}</div>
              </div>
